...
results = {}
for i in ["max_cells", "max_pop"]:
    if i == "max_cells":
        population_density = np.array([1 for pop_cell_centroid in population_cells_centroids.values.tolist()])
    else:
    ...
    for network in ["active", "not_active"]:
        ...
        for charging_airports_lim in range(8, 16):
            ...
            results[i][network][1].append(len(population_covered))
            population = np.array([pop_cell_centroid[1] for pop_cell_centroid in
                                   population_cells_centroids.values.tolist()])
            results[i][network][2].append(sum([population[pop_id] for pop_id in population_covered]))
            ...
            if not settings.show_plot:
                ...
                plot_name = "test_{}_{}".format(settings.airports_config.charging_bases_lim,
