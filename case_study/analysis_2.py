...
_logger.info("-------------- EACN-REG SWE analysis to investigate the trade-off between total travel time threshold and"
...
results = {}
settings.airports_config.charging_bases_lim = 100
population_density = np.array([pop_cell_centroid[2] for pop_cell_centroid in
                               population_cells_centroids.values.tolist()])
for total_time in [3, 4]:
    results[total_time] = {}
    settings.paths_config.max_total_time_travel = total_time
    for tau in [400, 600]:
        ...
        for network in ["active", "non_active"]:
            ...
            airports = get_airports(airports_file=airports_file, only_active=True if network == "active" else False)
            ...
            results[total_time][tau][network][0] = len(charging_airports)
            results[total_time][tau][network][1] = len(population_covered)
            population = np.array([pop_cell_centroid[1] for pop_cell_centroid in
                                   population_cells_centroids.values.tolist()])
            results[total_time][tau][network][2] = sum([population[pop_id] for pop_id in population_covered])
...
pickle.dump(results, open('analysis_2.pkl', 'wb'))
